...
<script>
	$(function() {
       $("#grid").treegrid({
    	   onDblClickRow:function(index,row){
    		   var url = '${ctx}/system/sys-department?method=view&id=' + row.id;
	...
	var toolbar = [{
		...
		handler:function(){
			...
			var url = '${ctx}/system/sys-department!input.action';
		...
		handler:function(){
			...
			var url = '${ctx}/system/sys-department!input.action?id=' + row.id;
		...
		handler:function(){
			...
			$.messager.confirm('确认对话框', '确认你是否要删除数据？', function(r) {
				if (r) {
					$.post("${ctx}/system/sys-department!delete.action", {
						'id':row.id
					}, function(data) {
						if (data.status) {
...
<body>
	<table id="grid" title="部门列表管理" style="width: 100%; height: 100%"
		data-options="idField:'id',treeField:'departmentName',fit:true,fitColumns:true,singleSelect:false,rownumbers:true,url:'${ctx}/system/sys-department!tree.action',pagination:false,toolbar:toolbar,checkbox:true,checkOnSelect:true">
		<thead>
			<tr>
